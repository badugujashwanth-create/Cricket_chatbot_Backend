...
function formatRate(value = 0) {
  return Number(round(Number(value || 0), 2)).toFixed(2);
}

function buildScopeLabel(filters = {}) {
  const bits = [];
  if (filters.season) bits.push(`Season ${filters.season}`);
  if (filters.format) bits.push(String(filters.format));
  if (filters.venue) bits.push(String(filters.venue));
  return bits.length ? ` (${bits.join(', ')})` : '';
}

function buildPlayerSummaryLine(player, stats, filters = {}) {
  const scope = buildScopeLabel(filters);
  return `Summary: ${player.name} has ${formatCount(stats.runs)} runs and ${formatCount(stats.wickets)} wickets in the verified archive${scope ? scope.toLowerCase() : ''}.`;
}

function buildPlayerAnswer(player, stats, filters = {}) {
  const contextText = buildScopeLabel(filters);
  return [
    `${player.name}${contextText}`,
    '',
    `Matches: ${formatCount(stats.matches)}`,
    `Runs: ${formatCount(stats.runs)}`,
    `Average: ${formatRate(stats.average)}`,
    `Strike Rate: ${formatRate(stats.strike_rate)}`,
    `Wickets: ${formatCount(stats.wickets)}`,
    `Economy: ${formatRate(stats.economy)}`,
    '',
    buildPlayerSummaryLine(player, stats, filters)
  ].join('\n');
}

function buildCompareConclusion(left, leftStats, right, rightStats) {
  const points = [];
  if (leftStats.runs !== rightStats.runs) {
    points.push(
      `${leftStats.runs > rightStats.runs ? left.name : right.name} leads on total runs`
    );
  }
  if (leftStats.average !== rightStats.average) {
    points.push(
      `${leftStats.average > rightStats.average ? left.name : right.name} has the stronger batting average`
    );
  }
  if (leftStats.strike_rate !== rightStats.strike_rate) {
    points.push(
      `${leftStats.strike_rate > rightStats.strike_rate ? left.name : right.name} scores faster by strike rate`
    );
  }
  if (leftStats.wickets !== rightStats.wickets) {
    points.push(
      `${leftStats.wickets > rightStats.wickets ? left.name : right.name} contributes more wickets`
    );
  }
  if (!points.length) {
    return 'Conclusion: Both players are level on the compared metrics in this sample.';
  }
  return `Conclusion: ${points.join(', ')}.`;
}

function buildComparePlayersAnswer(comparison, leftStats, rightStats, filters = {}) {
  const scope = buildScopeLabel(filters);
  return [
    `${comparison.left.name} vs ${comparison.right.name}${scope}`,
    '',
    'Matches:',
    `${comparison.left.name}: ${formatCount(leftStats.matches)}`,
    `${comparison.right.name}: ${formatCount(rightStats.matches)}`,
    '',
    'Batting Average:',
    `${comparison.left.name}: ${formatRate(leftStats.average)}`,
    `${comparison.right.name}: ${formatRate(rightStats.average)}`,
    '',
    'Strike Rate:',
    `${comparison.left.name}: ${formatRate(leftStats.strike_rate)}`,
    `${comparison.right.name}: ${formatRate(rightStats.strike_rate)}`,
    '',
    'Runs:',
    `${comparison.left.name}: ${formatCount(leftStats.runs)}`,
    `${comparison.right.name}: ${formatCount(rightStats.runs)}`,
    '',
    'Wickets:',
    `${comparison.left.name}: ${formatCount(leftStats.wickets)}`,
    `${comparison.right.name}: ${formatCount(rightStats.wickets)}`,
    '',
    buildCompareConclusion(comparison.left, leftStats, comparison.right, rightStats)
  ].join('\n');
}

function buildHeadToHeadConclusion(team1Name, team2Name, result) {
  if (result.wins_team_a === result.wins_team_b) {
    return `Conclusion: ${team1Name} and ${team2Name} are level on wins in this head-to-head sample.`;
  }
  const leader = result.wins_team_a > result.wins_team_b ? team1Name : team2Name;
  return `Conclusion: ${leader} has the stronger head-to-head record in the verified archive sample.`;
}

function buildTopRowsAnswer(label, rows = [], entities = {}) {
  ...
  return [
    `Top ${label}${scope}`,
    '',
    ...rows.slice(0, 5).map((row, index) => `${index + 1}. ${row.player} - ${row.value}`),
    '',
    `Summary: ${rows[0].player} ranks first for ${label} in the current query scope.`
  ].join('\n');
}
...
function executePlayerStats({ playerId, filters = {}, action = 'player_stats' }) {
  ...
  return {
    answer: buildPlayerAnswer(summary, safe, filters),
    data: {
      type: 'player_stats',
      player: {
        id: summary.id,
...
function executeTeamStats({ teamId, filters = {}, action = 'team_stats' }) {
  ...
  return {
    answer: [
      `${summary.name}${contextText}`,
      '',
      `Matches: ${formatCount(safe.matches)}`,
      `Wins: ${formatCount(safe.wins)}`,
      `Win Rate: ${formatRate(safe.win_rate)}%`,
      `Average Score: ${formatRate(safe.average_score)}`,
      '',
      `Summary: ${summary.name} won ${formatCount(safe.wins)} of ${formatCount(safe.matches)} matches in the verified archive scope.`
    ].join('\n'),
    data: {
      type: 'team_stats',
...
function executeMatchSummary({ matchId, team1, team2, season, date }) {
  ...
  const topBowlers = (match.top_bowlers || [])
    ...
    .join(', ');
  const scoreLines = Array.isArray(match.score)
    ? match.score
        .slice(0, 2)
        .map((row) => {
          const wickets = row.wickets === null || row.wickets === undefined ? '' : `/${row.wickets}`;
          const overs = row.overs === null || row.overs === undefined ? '' : ` (${row.overs} overs)`;
          return `${row.inning || 'Innings'}: ${row.runs || 0}${wickets}${overs}`;
        })
    : [];
  const matchTitle = match.name || [team1, team2].filter(Boolean).join(' vs ') || 'Match Summary';

  return {
    answer: [
      `${matchTitle}${match.match_type ? ` (${match.match_type})` : ''}`,
      '',
      ...scoreLines,
      `Venue: ${match.venue || 'Venue unavailable'}`,
      `Result: ${match.result || 'Result unavailable'}`,
      '',
      `Top Batters: ${topBatters || 'Not available'}`,
      `Top Bowlers: ${topBowlers || 'Not available'}`,
      '',
      `Summary: ${match.result || 'Match details are available from the verified archive.'}`
    ].join('\n'),
    data: {
      type: 'match_summary',
      match
...
function executeComparePlayers({ playerId1, playerId2, filters = {} }) {
  ...
  return {
    answer: buildComparePlayersAnswer(comparison, leftStats, rightStats, filters),
    data: {
...
function executeHeadToHead({ team1Name, team2Name, filters = {} }) {
  ...
  return {
    answer: [
      `${team1Name} vs ${team2Name}${buildScopeLabel(filters)}`,
      '',
      `Matches: ${formatCount(result.matches)}`,
      `${team1Name} Wins: ${formatCount(result.wins_team_a)}`,
      `${team2Name} Wins: ${formatCount(result.wins_team_b)}`,
      `No Result: ${formatCount(result.no_result)}`,
      '',
      buildHeadToHeadConclusion(team1Name, team2Name, result)
    ].join('\n'),
    data: {
      type: 'head_to_head',
      team1: team1Name,
...
function executeTopList({ entities = {} }) {
  ...
  return {
    answer: buildTopRowsAnswer(label, rows, entities),
    data: {
      type: 'top_players',
      metric,
      rows

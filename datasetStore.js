...
const CSV_PATH = path.join(__dirname, 'cleaned_balls_all_matches.csv');
...
function serializeCachePart(value) {
  ...
  if (Array.isArray(value)) return value.map((item) => serializeCachePart(item)).join(',');
  return String(value);
}

function buildResultCacheKey(scope = '', params = {}) {
  const entries = Object.entries(params)
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([key, value]) => `${key}=${serializeCachePart(value)}`);
  return `${scope}:${entries.join('&')}`;
}
...
function setCachedResult(key = '', value) {
  if (!key) return value;
  if (state.resultCache.has(key)) {
    state.resultCache.delete(key);
  }
  state.resultCache.set(key, value);
  if (state.resultCache.size > RESULT_CACHE_LIMIT) {
...
function buildSearchIndexKeys(value = '') {
  ...
  for (const token of tokens) {
    ...
    if (token.length >= SEARCH_INDEX_PREFIX_LENGTH) {
      keys.add(token.slice(0, SEARCH_INDEX_PREFIX_LENGTH));
    }
  }
  return [...keys];
}
...
function addIndexValue(index, key, value) {
  if (!key) return;
  getOrCreate(index, key, () => []).push(value);
}

function intersectLists(lists = [], idAccessor = (item) => item?.id) {
  if (!lists.length) return [];
  if (lists.length === 1) return lists[0];

  const sorted = [...lists].sort((a, b) => a.length - b.length);
  const [base, ...rest] = sorted;
  const lookups = rest.map((list) => new Set(list.map((item) => idAccessor(item))));
  return base.filter((item) => {
    const id = idAccessor(item);
    return lookups.every((lookup) => lookup.has(id));
  });
}

function getSeasonCandidates(bySeason = new Map(), seasonQuery = '') {
  if (!seasonQuery) return [];
  const direct = bySeason.get(seasonQuery);
  if (direct) return direct;

  const items = [];
  const seen = new Set();
  for (const [key, rows] of bySeason.entries()) {
    if (!key.includes(seasonQuery)) continue;
...
function passesMatchFilters(match = {}, filters = {}) {
  ...
  const matchMeta = state.cache?.matchSearchMetaById?.get(matchId);

  if (formatFilter && (matchMeta?.format_key || normalizeText(match.format || '')) !== formatFilter) {
    return false;
  }
  if (seasonFilter && !seasonIncludes(match.season || '', seasonFilter)) return false;
  if (venueFilter && (matchMeta?.venue_key || normalizeText(match.venue || '')) !== venueFilter) {
    return false;
  }
  if (teamFilter) {
    const teamKeys =
      matchMeta?.team_keys ||
      (Array.isArray(match.teams) ? match.teams.map((team) => normalizeText(team)) : []);
    if (!teamKeys.includes(teamFilter)) return false;
  }
  return true;
}
...
function buildCache(raw) {
  ...
  const matchSearchMetaById = new Map();
  const matchesByTeam = new Map();
  const matchesBySeason = new Map();
  const matchesByVenue = new Map();

  for (const match of matches) {
    const teamKeys = (Array.isArray(match.teams) ? match.teams : []).map((team) => normalizeText(team));
    const seasonKey = normalizeText(match.season || '');
    const venueKey = normalizeText(match.venue || '');
    const formatKey = normalizeText(match.format || '');
    matchSearchMetaById.set(match.id, {
      team_keys: teamKeys,
      season_key: seasonKey,
      venue_key: venueKey,
      format_key: formatKey
    });
    teamKeys.forEach((key) => addIndexValue(matchesByTeam, key, match));
    addIndexValue(matchesBySeason, seasonKey, match);
    addIndexValue(matchesByVenue, venueKey, match);
  }

  const playerSearchMetaById = new Map();
  const playerSearchIndex = new Map();
  const playerMapByNormalizedName = new Map();
  const players = [...raw.players.values()].map((player) => {
    ...
    const searchKeys = buildSearchIndexKeys(player.name || '');
    playerSearchMetaById.set(item.id, {
      name_key: nameKey,
      search_keys: searchKeys
    });
    if (nameKey && !playerMapByNormalizedName.has(nameKey)) {
      playerMapByNormalizedName.set(nameKey, item);
    }
    searchKeys.forEach((key) => addIndexValue(playerSearchIndex, key, item));
    return item;
  });

  const teamMapByNormalizedName = new Map();
  const teams = [...raw.teams.values()].map((team) => {
    ...
    if (teamKey && !teamMapByNormalizedName.has(teamKey)) {
      teamMapByNormalizedName.set(teamKey, item);
    }
    return item;
  });
  ...
  return {
    ...
    matches,
    playerMapById,
    playerMapByNormalizedName,
    teamMapById,
    teamMapByNormalizedName,
    matchMap,
    playerSearchMetaById,
    playerSearchIndex,
    matchSearchMetaById,
    matchIndexes: {
      byTeam: matchesByTeam,
      bySeason: matchesBySeason,
      byVenue: matchesByVenue
    },
    venues,
    formats
  };
...
function getPlayerSearchCandidates(query = '') {
  if (!state.cache) return [];
  const cleanQuery = normalizeText(query || '');
  if (!cleanQuery) return state.cache.players;

  const exact = state.cache.playerMapByNormalizedName?.get(cleanQuery);
  if (exact) return [exact];

  const candidateMap = new Map();
  buildSearchIndexKeys(cleanQuery).forEach((key) => {
    const items = state.cache.playerSearchIndex?.get(key) || [];
    items.forEach((item) => candidateMap.set(item.id, item));
  });
  return candidateMap.size ? [...candidateMap.values()] : state.cache.players;
}

function searchPlayers({ q = '', page = 1, limit = 20 } = {}) {
  if (!state.cache) return null;
  const cleanQuery = String(q || '').trim();
  const cleanPage = Math.max(1, Number(page) || 1);
  const cleanLimit = Math.min(50, Math.max(1, Number(limit) || 20));
  const cacheKey = buildResultCacheKey('players:search', {
    q: normalizeText(cleanQuery),
    page: cleanPage,
    limit: cleanLimit
  });
  ...
  const source = cleanQuery
    ? rankByQuery(getPlayerSearchCandidates(cleanQuery), cleanQuery, (player) => player.name)
    : state.cache.players;
  ...
  return setCachedResult(cacheKey, {
    items,
    pagination: {
      page: cleanPage,
      limit: cleanLimit,
...
function getMatches({ team = '', season = '', venue = '', limit = 10, offset = 0 } = {}) {
  ...
  const seasonNorm = normalizeText(seasonText);
  const venueNorm = normalizeText(venue);
  const cacheKey = buildResultCacheKey('matches:list', {
    team: teamNorm,
    season: seasonNorm,
    venue: venueNorm,
    limit: cleanLimit,
  ...
  const indexedLists = [];
  if (teamNorm) {
    indexedLists.push(state.cache.matchIndexes?.byTeam?.get(teamNorm) || []);
  }
  if (seasonNorm) {
    indexedLists.push(getSeasonCandidates(state.cache.matchIndexes?.bySeason, seasonNorm));
  }
  if (venueNorm) {
    indexedLists.push(state.cache.matchIndexes?.byVenue?.get(venueNorm) || []);
  }

  const source = indexedLists.length
    ? intersectLists(indexedLists, (match) => match.id)
    : state.cache.matches;
  const filtered = source.filter((match) => passesMatchFilters(match, {
    team,
    season,
    venue
  }));
  ...
  const totalPages = Math.max(1, Math.ceil(total / cleanLimit));
  const page = Math.floor(cleanOffset / cleanLimit) + 1;
  return setCachedResult(cacheKey, {
...
function getPlayerSummary(playerId, filters = {}) {
  if (!state.cache) return null;
  const summaryKey = buildResultCacheKey('players:summary', {
    id: String(playerId || '').trim(),
    team: normalizeText(filters.team || ''),
    season: normalizeText(filters.season || ''),
    format: normalizeText(filters.format || ''),
    venue: normalizeText(filters.venue || '')
  });
  const cached = getCachedResult(summaryKey);
  if (cached) return cached;
  ...
  return setCachedResult(summaryKey, {
    id: player.id,
    name: player.name,
    team: player.team,
    stats
...
function getTeamSummary(teamId, filters = {}) {
  if (!state.cache) return null;
  const summaryKey = buildResultCacheKey('teams:summary', {
...
function searchTeams(query = '') {
  ...
  const cacheKey = buildResultCacheKey('teams:search', {
    q: normalizeText(cleanQuery)
  });
  const cached = getCachedResult(cacheKey);
  if (cached) return cached;

  const ranked = rankByQuery(state.cache.teams, cleanQuery, (team) => team.name);
  const source = cleanQuery ? ranked : state.cache.teams;
  return setCachedResult(cacheKey, source.map((team) => ({
    id: team.id,
    name: team.name
  })));
}
